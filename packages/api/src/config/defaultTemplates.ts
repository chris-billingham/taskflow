/**
 * Built-in public templates shown in the Template Gallery's default tab.
 *
 * These live in src/ (not prisma/seed.ts) because production installs run
 * `prisma migrate deploy` and never the seed script — so the gallery opened
 * empty on every real deployment. ensureDefaultTemplates() applies them at
 * API boot; the seed script imports the same list for local development.
 */
